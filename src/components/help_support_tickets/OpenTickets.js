    ...
    <div className='grid grid-cols-1 sm:grid-cols-2 gap-5 my-4'>
        {dataArray.map((data)=>(
            <div key={data.ID} className={` p-5 shd rounded-lg`}>
                ...
                <button onClick={()=>setView(true)} className='rounded-lg border border-blue text-blue hover:bg-blue hover:text-white font-medium py-[3px] px-4 text-sm transition-all duration-300'>View</button>
                </div>
